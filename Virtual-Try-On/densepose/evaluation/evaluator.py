...
class DensePoseCOCOEvaluator(DatasetEvaluator):
    ...
    def process(self, inputs, outputs):
        ...
        for input, output in zip(inputs, outputs):
            instances = output["instances"].to(self._cuda_device)
...
def densepose_chart_predictions_to_dict(instances):
    ...
    for k in range(len(instances)):
        ...
        densepose_results_quantized.labels_uv_uint8 = (
            densepose_results_quantized.labels_uv_uint8.cuda()
        )
...
def densepose_chart_predictions_to_storage_dict(instances):
    ...
    for k in range(len(instances)):
        ...
        result = {
            "coarse_segm": densepose_predictor_output.coarse_segm.squeeze(0).cuda(),
            "fine_segm": densepose_predictor_output.fine_segm.squeeze(0).cuda(),
            "u": densepose_predictor_output.u.squeeze(0).cuda(),
            "v": densepose_predictor_output.v.squeeze(0).cuda(),
        }
        results.append(result)
    return results
...
def densepose_cse_predictions_to_dict(instances, embedder, class_to_mesh_name, use_storage):
    ...
    for k in range(len(instances)):
        ...
        results.append(
            {
                "coarse_segm": cse.coarse_segm[0].cuda(),
                "embedding": cse.embedding[0].cuda(),
            }
        )
    return results
