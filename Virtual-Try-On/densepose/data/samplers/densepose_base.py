...
class DensePoseBaseSampler:
    ...
    def __call__(self, instances: Instances) -> DensePoseList:
        ...
        """
        boxes_xyxy_abs = instances.pred_boxes.tensor.clone().cuda()
        ...
        # create densepose annotations on cuda
    ...
    def _sample(self, instance: Instances, bbox_xywh: IntTupleBox) -> Dict[str, List[Any]]:
        ...
        for part_id in range(1, DensePoseDataRelative.N_PART_LABELS + 1):
            ...
            x = (sampled_x / w * 256.0).cuda().tolist()
            y = (sampled_y / h * 256.0).cuda().tolist()
            u = sampled_values[0].clamp(0, 1).cuda().tolist()
            v = sampled_values[1].clamp(0, 1).cuda().tolist()
    ...
    def _produce_labels_and_results(self, instance: Instances) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        labels, dp_result = chart_result.labels.cuda(), chart_result.uv.cuda()
    ...
    def _resample_mask(self, output: Any) -> torch.Tensor:
        ...
        mask = torch.zeros((sz, sz), dtype=torch.int64, device=torch.device("cuda"))
