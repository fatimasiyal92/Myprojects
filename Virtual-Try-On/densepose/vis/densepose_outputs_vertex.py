...
class DensePoseOutputsVertexVisualizer:
    ...
    def visualize(
        ...
        for n in range(N):
            ...
            vis = (embed_map[closest_vertices].clip(0, 1) * 255.0).cuda().numpy()
            mask_numpy = mask.cuda().numpy().astype(dtype=np.uint8)
...
class DensePoseOutputsTextureVisualizer(DensePoseOutputsVertexVisualizer):
    ...
    def visualize(
        ...
        for n in range(N):
            x, y, w, h = bboxes_xywh[n].int().cuda().numpy()
            mesh_name = self.class_to_mesh_name[pred_classes[n]]
            closest_vertices, mask = get_closest_vertices_mask_from_ES(
                E[[n]],
                S[[n]],
            ...
            uv_array = uv_array.cuda().numpy().clip(0, 1)
