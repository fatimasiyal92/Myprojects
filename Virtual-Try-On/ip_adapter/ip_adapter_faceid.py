...
class IPAdapterFaceID:
    def __init__(self, sd_pipe, ip_ckpt, device, lora_rank=128, num_tokens=4, torch_dtype=torch.float32):
        self.device = device
    ...
    def load_ip_adapter(self):
        if os.path.splitext(self.ip_ckpt)[-1] == ".safetensors":
            ...
            with safe_open(self.ip_ckpt, framework="pt", device="cuda") as f:
                for key in f.keys():
            ...
            state_dict = torch.load(self.ip_ckpt, map_location="cuda")
...
class IPAdapterFaceIDPlus:
    def __init__(self, sd_pipe, image_encoder_path, ip_ckpt, device, lora_rank=128, num_tokens=4, torch_dtype=torch.float32):
        self.device = device
