...
class BitMasks:
    ...
    def __init__(self, tensor: Union[torch.Tensor, np.ndarray]):
        ...
        Args:
            tensor: bool Tensor of N,H,W, representing N instances in the image.
        """
        device = tensor.device if isinstance(tensor, torch.Tensor) else torch.device("cuda")
...
class PolygonMasks:
    ...
    def __init__(self, polygons: List[List[Union[torch.Tensor, np.ndarray]]]):
        ...
        def _make_array(t: Union[torch.Tensor, np.ndarray]) -> np.ndarray:
            ...
            # Always put polygons on cuda (self.to is a no-op) since they
    ...
    def __getitem__(self, item: Union[int, slice, List[int], torch.BoolTensor]) -> "PolygonMasks":
        ...
        if isinstance(item, int):
            ...
            # Polygons is a list, so we have to move the indices back to cuda.
            if item.dtype == torch.bool:
                assert item.dim() == 1, item.shape
                item = item.nonzero().squeeze(1).cuda().numpy().tolist()
            elif item.dtype in [torch.int32, torch.int64]:
                item = item.cuda().numpy().tolist()
            else:
    ...
    def crop_and_resize(self, boxes: torch.Tensor, mask_size: int) -> torch.Tensor:
        ...
        device = boxes.device
        # Put boxes on the cuda, as the polygon representation is not efficient GPU-wise
