...
def to_device(t, device_str):
    ...
    if src == dst:
        ...
        return torch.ops._caffe2.CopyGPUTocuda(t)
    elif src.type == "cuda" and dst.type == "cuda":
        return torch.ops._caffe2.CopycudaToGPU(t)
    else:
...
def get_params_from_init_net(
    ...
    # only exception be CopyGPUTocuda which is cuda op but returns cuda tensor.
    def _get_device_option(producer_op):
        if producer_op.type == "CopyGPUTocuda":
...
def _generic_status_identifier(
    ...
    Statically infer the status of each blob, the status can be such as device type
        (cuda/GPU), layout (NCHW/NHWC), data type (float32/int8), etc. "Blob" here
...
def infer_device_type(
    ...
    def _copy_cuda_to_gpu_updater(op, input_types, output_types):
        if input_types[0] == _GPU_STR or output_types[0] == _cuda_STR:
            _updater_raise(op, input_types, output_types)
        return ([_cuda_STR], [_GPU_STR])

    def _copy_gpu_to_cuda_updater(op, input_types, output_types):
        if input_types[0] == _cuda_STR or output_types[0] == _GPU_STR:
            _updater_raise(op, input_types, output_types)
        return ([_GPU_STR], [_cuda_STR])
    ...
    def _device_updater(op, *args, **kwargs):
        return {
            "CopycudaToGPU": _copy_cuda_to_gpu_updater,
            "CopyGPUTocuda": _copy_gpu_to_cuda_updater,
...
def fuse_copy_between_cuda_and_gpu(predict_net: caffe2_pb2.NetDef):
    """
    In-place fuse extra copy ops between cuda/gpu for the following case:
    ...
    _COPY_OPS = ["CopycudaToGPU", "CopyGPUTocuda"]
