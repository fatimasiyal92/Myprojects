...
def _do_paste_mask(masks, boxes, img_h, img_w, skip_empty=True):
    ...
    Args:
        ...
        skip_empty (bool): only paste masks within the region that
            ...
            An important optimization for cuda.
...
def paste_masks_in_image(masks, boxes, image_shape, threshold=0.5):
    ...
    if device.type == "cuda":
        # cuda is most efficient when they are pasted one by one with skip_empty=True
...
# It is faster on cuda, while the aligned one is faster on GPU thanks to grid_sample.
...
def paste_mask_in_image_old(mask, box, img_h, img_w, threshold):
    ...
    mask = Image.fromarray(mask.cuda().numpy())
