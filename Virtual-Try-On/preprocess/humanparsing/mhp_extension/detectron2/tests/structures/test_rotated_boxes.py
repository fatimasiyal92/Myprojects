...
class TestRotatedBoxesLayer(unittest.TestCase):
    ...
    def test_iou_0_dim_cuda(self):
        ...
        self.assertTrue(torch.allclose(ious_cuda.cuda(), expected_ious))
    ...
    def test_iou_precision(self):
        for device in ["cuda"] + ["cuda"] if torch.cuda.is_available() else []:
            ...
            ious = pairwise_iou_rotated(boxes1, boxes2)
            self.assertTrue(torch.allclose(ious.cuda(), expected_ious))
...
def benchmark_rotated_iou():
    ...
    args = [{"dev": torch.device("cuda"), "n": 1}]
