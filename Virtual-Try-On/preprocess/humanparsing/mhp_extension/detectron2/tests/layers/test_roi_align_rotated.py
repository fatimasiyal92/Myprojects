...
class ROIAlignRotatedTest(unittest.TestCase):
    ...
    def _simple_roi_align_rotated(self, img, box, resolution):
        ...
        result_cuda = op.forward(input, rois)
        if torch.cuda.is_available():
            result_cuda = op.forward(input.cuda(), rois.cuda())
            assert torch.allclose(result_cuda, result_cuda.cuda())
        return result_cuda[0, 0]
    ...
    def test_roi_align_rotated_gradcheck_cuda(self):
        dtype = torch.float64
        ...
        assert gradcheck(func, (x,)), "gradcheck failed for RoIAlignRotated cuda"
        assert gradcheck(func, (x.transpose(2, 3),)), "gradcheck failed for RoIAlignRotated cuda"
