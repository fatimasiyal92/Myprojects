...
class DensePoseResult(object):
    def __init__(self, boxes_xywh, S, I, U, V):
        self.results = []
        self.boxes_xywh = boxes_xywh.cuda().tolist()
        ...
        for i, box_xywh in enumerate(boxes_xywh):
            ...
            result_numpy_i = result_i.cuda().numpy()
...
class DensePoseList(object):
    ...
    _TORCH_DEVICE_cuda = torch.device("cuda")

    def __init__(self, densepose_datas, boxes_xyxy_abs, image_size_hw, device=_TORCH_DEVICE_cuda):
