...
def onnx_inference(session, lip_session, input_dir):
    ...
    with torch.no_grad():
        for _, batch in enumerate(tqdm(dataloader)):
            ...
            logits_result = transform_logits(upsample_output.data.cuda().numpy(), c, s, w, h, input_size=[512, 512])
            parsing_result = np.argmax(logits_result, axis=2)
            parsing_result = np.pad(parsing_result, pad_width=1, mode='constant', constant_values=0)
        ...
        with torch.no_grad():
            for _, batch in enumerate(tqdm(dataloader_lip)):
                ...
                logits_result_lip = transform_logits(upsample_output_lip.data.cuda().numpy(), c, s, w, h,
                                                     input_size=[473, 473])
